...
class SimulationApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.browse_label = CustomLabel("") # Initially it's label has nothing to show up
        ...
        self.start_time_input.setFocus() # To set focus on start_time when window opens initially
        ...
        # Button to validate the inputs and execute model to show the plot_simulation button
    ...
    def show_plot_button(self):
        """
        To Display the Plot Simulation Button after the Simulation is executed successfully.
        """
        self.grid_layout.addWidget(self.plot_button, 0, 0, 1, 2, alignment=Qt.AlignmentFlag.AlignHCenter)

    def browse_file(self):
        """
        Open a file dialog to select the executable file created from the OpenModelica model.
        """
    ...
    def execute_model(self):
        """
        Button to execute the selected simulation with specified start and stop time.
        """
    ...
    def trigger_finished(self, log:str, message:object):
        ...
        if log == "LOG_SUCCESS":
            ...
            Popup("SUCCESS", str(message) + "\nOpen Plot to see the Simulation Plots", "success")
        else:
            # Show error message
            Popup("ERROR", "Please! select an appropriate exe\n" + str(message), "error")
